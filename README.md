...
### V1.4 
合并所有采样频率为12kHz的csv。模型主体架构不变，使用Codex，以提升模型的准确率为目标。换而言之，面对多工况，该模型的泛化能力更强。
| 指标 | 结果 |
|---|---:|
...
6. 模型准确度结果与CWT图像质量、模型层数，损失函数等密切相关，特别是CWT图像质量。
7. FD-MVLLM为故障诊断模型，适用的数据集为JNU,CWRU,PU，轴承剩余使用寿命（RUL）研究领域的数据集有待验证。
8. 
