...
def plot_tsne(features, labels, title="t-SNE Visualization", save_path=r"./results/tsne.csv"):
    """执行t-SNE并绘制结果"""
    # 数据标准化
    scaler = st()
...
def get_features(model, dataloader, device):
    ...
    with torch.no_grad():
        for batch_csv, batch_images, batch_y in dataloader:
            ...
            _ = model.classify(batch_csv, batch_images)
